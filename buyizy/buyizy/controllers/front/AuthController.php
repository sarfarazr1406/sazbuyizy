...
class AuthControllerCore extends FrontController
{
	...
	protected function processSubmitAccount()
	{
		...
		if (Tools::isSubmit('submitAccount'))
		{
			if (empty($passwd_confirm))
				$this->errors[] = Tools::displayError('Confirm Password is empty');
			elseif ($passwd != $passwd_confirm)
				$this->errors[] = Tools::displayError('Your Password and Confirm password do not match.');
			$this->context->smarty->assign('email_create', 1);
		}
		// New Guest customer
	...
	protected function sendConfirmationMail(Customer $customer)
	{
		...
		$voucher = $this->createDiscount((int)$customer->id);
