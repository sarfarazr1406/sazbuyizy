...
include_once(dirname(__FILE__).'../../../referralcode.php');

class referralcodecodeModuleFrontController extends ModuleFrontController
{
    ...
    public function init()
    {
        if (!$this->context->customer->isLogged())
            Tools::redirect($this->context->link->getPageLink('authentication', true, (int)$this->context->language->id, 'back='
                .urlencode($this->context->link->getModuleLink('referralcode', 'code'))));
    ...
    public function initContent()
    {
        ...
        $discount_type_sponsored = (int)(Configuration::get('REFERRAL_DISCOUNT_TYPE_SPONSORED'));
		$discount_type_sponsorer = (int)(Configuration::get('REFERRAL_DISCOUNT_TYPE_SPONSORER'));

        if ($discount_type_sponsored == 1)
            $discount_sponsored = Discount::display((float)(Configuration::get('REFERRAL_PERCENTAGE_SPONSORED')), $discount_type_sponsored, new Currency($this->context->currency->id));
        else
            $discount_sponsored = Discount::display((float)(Configuration::get('REFERRAL_DISCOUNT_VALUE_SPONSORED_'.(int)($this->context->currency->id))), $discount_type_sponsored, new Currency($this->context->currency->id));
		
		if ($discount_type_sponsorer == 1)
            $discount_sponsorer = Discount::display((float)(Configuration::get('REFERRAL_PERCENTAGE_SPONSORER')), $discount_type_sponsorer, new Currency($this->context->currency->id));
        else
            $discount_sponsorer = Discount::display((float)(Configuration::get('REFERRAL_DISCOUNT_VALUE_SPONSORER_'.(int)($this->context->currency->id))), $discount_type_sponsorer, new Currency($this->context->currency->id));
        ...
        if (Tools::isSubmit('submitSponsorFriends') AND Tools::getValue('friendsEmail') AND sizeof($friendsEmail = Tools::getValue('friendsEmail')) >= 1)
        {
            ...
            if (isset($result['selfCode']) && !empty($result['selfCode'])) {
            ...
            else
            {
                ...
                foreach ($friendsEmail AS $key => $friendEmail)
                {
                    ...
                    else
                    {
                        ...
                        if(!$selfCode)
                            $referralprogram->sponsorCode = $selfCode;
                        ...
                        {                            
                            ...
                            {                                
                                ...
                                $vars = array(
                                    ...
                                    '{link}' => Context::getContext()->link->getPageLink('authentication', true, Context::getContext()->language->id, 'create_account=1&sponsor='.urlencode($cipherTool->encrypt($referralprogram->id.'|'.$selfCode.'|')).'&back=my-account', false),
                                    '{discount_amt}' => $discount_sponsored);
            ...
            if (Tools::getValue('friendChecked') AND sizeof($friendsChecked = Tools::getValue('friendChecked')) >= 1)
            {
                foreach ($friendsChecked as $key => $friendChecked)
                {
                    ...
                    $vars = array(
                        ...
                        '{discount}' => $discount_sponsored
                    );
                    $referralprogram->save();
                    Mail::Send((int)$this->context->language->id, 'referralprogram-invitation', Mail::l('Referral Program', (int)$this->context->language->id), $vars, $referralprogram->selfCode, $referralprogram->firstname.' '.$referralprogram->lastname, strval(Configuration::get('PS_SHOP_EMAIL')), strval(Configuration::get('PS_SHOP_NAME')), NULL, NULL, dirname(__FILE__).'/../../mails/');
                    $revive_sent = true;
                    $nbRevive++;
                }
            }
        ...
        $discountInPercent = Tools::getValue('discount_type', Configuration::get('REFERRAL_DISCOUNT_TYPE_SPONSORED')) == 1;

        // Smarty display
        $this->context->smarty->assign(array(
            'activeTab' => $activeTab,
            'discount_sponsored' => $discount_sponsored,
			'discount_sponsorer' => $discount_sponsorer,
