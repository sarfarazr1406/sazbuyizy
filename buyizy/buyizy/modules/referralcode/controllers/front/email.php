 ...
 * @since 1.5.0
 */
class ReferralcodeEmailModuleFrontController extends ModuleFrontController
{
	...
	public function initContent()
	{
		...
		$file_path = _PS_THEME_DIR_.'/../../mails/en/referral_invitation.html';        
		...
		if ($discount_type == 1)
			$file = str_replace('{discount}', Discount::display((float)(Configuration::get('REFERRAL_PERCENTAGE_SPONSORER')), $discount_type, new Currency($this->context->currency->id)), $file);
		else
			$file = str_replace('{discount}', Discount::display((float)(Configuration::get('REFERRAL_DISCOUNT_VALUE_SPONSORER_' . $this->context->currency->id)), $discount_type, new Currency($this->context->currency->id)), $file);
