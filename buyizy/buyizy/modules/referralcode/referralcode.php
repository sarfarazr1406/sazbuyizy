...
class ReferralCode extends Module
{
    public function __construct()
    {
        ...
        $this->version = '1.0.1';
        $this->author = 'Sarfaraz';

        $this->controllers = array('code');

        $this->bootstrap = true;
        parent::__construct();

        $this->confirmUninstall = $this->l('All sponsors and friends will be deleted. Are you sure you want to uninstall this module?');
        $this->displayName = $this->l('Customer referral code system');
        $this->description = $this->l('Integrate a referral program through a unique referral code system into your shop.');
        if (Configuration::get('REFERRAL_DISCOUNT_TYPE_SPONSORER') == 1 AND !Configuration::get('REFERRAL_PERCENTAGE_SPONSORER'))
            $this->warning = $this->l('Please specify an amount for referral program vouchers.');

        if ($this->id)
        {
            $this->_configuration = Configuration::getMultiple(array('REFERRAL_NB_FRIENDS', 'REFERRAL_ORDER_QUANTITY', 'REFERRAL_DISCOUNT_TYPE_SPONSORER', 'REFERRAL_DISCOUNT_VALUE_SPONSORER','REFERRAL_DISCOUNT_TYPE_SPONSORED', 'REFERRAL_DISCOUNT_VALUE_SPONSORED'));
    ...
    public function install($delete_params = true)
    {
        ...
        if (!parent::install() OR !Configuration::updateValue('REFERRAL_DISCOUNT_DESCRIPTION', $desc)
            OR !Configuration::updateValue('REFERRAL_ORDER_QUANTITY', 1) OR !Configuration::updateValue('REFERRAL_DISCOUNT_TYPE_SPONSORER', 2) OR !Configuration::updateValue('REFERRAL_DISCOUNT_TYPE_SPONSORED', 2)
        ...
        /*Generate referral code for all existing customers if no. of orders placed is more than one.*/
        ...
        foreach($result AS $customer) {
            ...
            INSERT INTO `'._DB_PREFIX_.'referralcode` 
                (`id_customer`,`selfCode`) VALUES ('.$customer[id_customer].', "'.$selfCode.'")
            ');
        }
        
        /* Define a default value for fixed amount vouchers, for each currency */
        foreach (Currency::getCurrencies() AS $currency) {
            Configuration::updateValue('REFERRAL_DISCOUNT_VALUE_SPONSORER_'.(int)($currency['id_currency']), 5);
			Configuration::updateValue('REFERRAL_DISCOUNT_VALUE_SPONSORED_'.(int)($currency['id_currency']), 5);
		}
        ...
        Configuration::updateValue('REFERRAL_PERCENTAGE_SPONSORED', 5);
		Configuration::updateValue('REFERRAL_PERCENTAGE_SPONSORER', 5);
    ...
    public function uninstall($delete_params = true)
    {
        $result = true;
        foreach (Currency::getCurrencies() AS $currency) {
            $result = $result AND Configuration::deleteByName('REFERRAL_DISCOUNT_VALUE_SPONSORED_'.(int)($currency['id_currency']));
			$result = $result AND Configuration::deleteByName('REFERRAL_DISCOUNT_VALUE_SPONSORER_'.(int)($currency['id_currency']));
		}
        ...
        OR !Configuration::deleteByName('REFERRAL_PERCENTAGE_SPONSORED') OR !Configuration::deleteByName('REFERRAL_PERCENTAGE_SPONSORER') OR !Configuration::deleteByName('REFERRAL_ORDER_QUANTITY')
        OR !Configuration::deleteByName('REFERRAL_DISCOUNT_TYPE_SPONSORED') OR !Configuration::deleteByName('REFERRAL_DISCOUNT_TYPE_SPONSORER') OR !Configuration::deleteByName('REFERRAL_NB_FRIENDS')
    ...
    {
        return Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'referralcode`;');
    }
    ...
    private function _postProcess()
    {
        ...
        foreach (Tools::getValue('discount_value_sponsored') AS $id_currency => $discount_value_sponsored)
            Configuration::updateValue('REFERRAL_DISCOUNT_VALUE_SPONSORED_'.(int)($id_currency), (float)($discount_value_sponsored));
		foreach (Tools::getValue('discount_value_sponsorer') AS $id_currency => $discount_value_sponsorer)
            Configuration::updateValue('REFERRAL_DISCOUNT_VALUE_SPONSORER_'.(int)($id_currency), (float)($discount_value_sponsorer));
        Configuration::updateValue('REFERRAL_TAX', (int)(Tools::getValue('discount_tax')));
        Configuration::updateValue('REFERRAL_DISCOUNT_TYPE_SPONSORED', (int)(Tools::getValue('discount_type_sponsored')));
		Configuration::updateValue('REFERRAL_DISCOUNT_TYPE_SPONSORER', (int)(Tools::getValue('discount_type_sponsorer')));
        Configuration::updateValue('REFERRAL_NB_FRIENDS', (int)(Tools::getValue('nb_friends')));
        Configuration::updateValue('REFERRAL_PERCENTAGE_SPONSORED', (int)(Tools::getValue('discount_value_percentage_sponsored')));
		Configuration::updateValue('REFERRAL_PERCENTAGE_SPONSORER', (int)(Tools::getValue('discount_value_percentage_sponsorer')));
    ...
    private function _postValidation()
    {
		...
		if (!is_array(Tools::getValue('discount_value_sponsored')))
            $this->_errors[] = $this->displayError($this->l('Discount value is invalid.'));
		else {
		    foreach (Tools::getValue('discount_value_sponsored') AS $id_currency => $discount_value_sponsored)
             if ($discount_value_sponsored == '')
                $this->_errors[] = $this->displayError(sprintf($this->l('Discount value for the currency #%d is empty.'), $id_currency));
             elseif (!Validate::isUnsignedFloat($discount_value_sponsored))
                $this->_errors[] = $this->displayError(sprintf($this->l('Discount value for the currency #%d is invalid.'), $id_currency));
		}
		if (!is_array(Tools::getValue('discount_value_sponsorer')))
            $this->_errors[] = $this->displayError($this->l('Discount value is invalid.'));
		else {
		    foreach (Tools::getValue('discount_value_sponsorer') AS $id_currency => $discount_value_sponsorer)
             if ($discount_value_sponsorer == '')
                $this->_errors[] = $this->displayError(sprintf($this->l('Discount value for the currency #%d is empty.'), $id_currency));
             elseif (!Validate::isUnsignedFloat($discount_value_sponsorer))
                $this->_errors[] = $this->displayError(sprintf($this->l('Discount value for the currency #%d is invalid.'), $id_currency));
		}
        if (!(int)(Tools::getValue('discount_type_sponsored')) OR !(int)(Tools::getValue('discount_type_sponsorer')) OR Tools::getValue('discount_type_sponsorer') < 1 OR Tools::getValue('discount_type_sponsorer') > 2 OR Tools::getValue('discount_type_sponsorer') < 1 OR Tools::getValue('discount_type_sponsorer') > 2)
        ...
        if ((int)(Tools::getValue('discount_type_sponsored')) === 1 OR (int)(Tools::getValue('discount_type_sponsorer')) === 1)
            if (!(int)(Tools::getValue('discount_value_percentage_sponsored')) OR (int)(Tools::getValue('discount_value_percentage_sponsored')) < 0 OR (int)(Tools::getValue('discount_value_percentage_sponsored')) > 100
		        OR !(int)(Tools::getValue('discount_value_percentage_sponsored')) OR (int)(Tools::getValue('discount_value_percentage_sponsorer')) < 0 OR (int)(Tools::getValue('discount_value_percentage_sponsorer')) > 100)
    ...
    */
    public function hookShoppingCart($params)
    {
        ...
        {
            $this->smarty->assign(array('discount_display' => ReferralCode::displayDiscount($cartRule->reduction_percent ? $cartRule->reduction_percent : $cartRule->reduction_amount, $cartRule->reduction_percent ? 1 : 2, new Currency($params['cookie']->id_currency)), 'discount' => $cartRule));
    ...
    */
    public function hookCreateAccountForm($params)
    {
        ...
        if ($explodeResult AND count($explodeResult) > 1 
            AND list($id_referralprogram, $selfCode) = $explodeResult 
            AND (int)($id_referralprogram) AND !empty($selfCode) 
            AND $id_referralprogram == ReferralProgramModule::isCodeExists($selfCode))
        {
        $sponsorCode = $selfCode;   //sponsorer's self code is sponsorCode for new customer.
            $referralprogram = new ReferralProgramModule($id_referralprogram);
            if (Validate::isLoadedObject($referralprogram))
            {
                /* hack for display referralprogram information in form */
                $_POST['sponsorCode'] = $sponsorCode;
    ...
    * Create a discount for the customer if sponsorised
    ...
    public function hookCreateAccount($params)
    {
        ...
        if (empty($postVars) OR !isset($postVars['sponsorCode']) OR empty($postVars['sponsorCode'])) {
			...
			$referralprogram->id_sponsor = 0;
			$referralprogram->selfCode = Tools::passwdGen(6);
			$referralprogram->email = $newCustomer->email;
			$referralprogram->sponsorCode = 0;
        ...
        $sponsorCode = $postVars['sponsorCode'];
		$sponsor = ReferralProgramModule::getByCode($sponsorCode);
        if ($sponsor != null)
        {
            ...
            if (!$id_referralprogram = ReferralProgramModule::isEmailExists($newCustomer->email, true, false))
            {
                ...
                $referralprogram->email = $newCustomer->email;
                $referralprogram->sponsorCode = $sponsorCode;
            ...
            if ($referralprogram->id_sponsor == $sponsor->id)
            {
                ...
                if ($referralprogram->registerDiscountForSponsored((int)$params['cookie']->id_currency))
                {
                    ...
                    {
                        $referralprogram->sponsorCode = $sponsorCode;
                        $referralprogram->selfCode = $cartRule->code;
						$referralprogram->save();
                        $data = array(
						    '{firstname}' => $newCustomer->firstname,
                            '{voucher_num}' => $cartRule->code,
                            '{voucher_amount}' => (Configuration::get('REFERRAL_DISCOUNT_TYPE_SPONSORED') == 2 ? Tools::displayPrice((float)Configuration::get('REFERRAL_DISCOUNT_VALUE_SPONSORED_'.(int)$this->context->currency->id), (int)Configuration::get('PS_CURRENCY_DEFAULT')) : (float)Configuration::get('REFERRAL_PERCENTAGE_SPONSORED').'%'));
    ...
    */
    public function hookAdminCustomers($params)
    {
        ...
        if ($id_referralprogram = ReferralProgramModule::isSponsorised((int)$customer->id, 'true'))
        {
            $referralprogram = new ReferralProgramModule((int)$id_referralprogram);
            $sponsor = new Customer((int)$referralprogram->id_sponsor);
            $customerCode = $referralprogram->selfCode;
        }
    ...
    */
    public function hookOrderConfirmation($params)
    {
        ...
        if ((int)$nbOrdersCustomer == (int)$this->_configuration['REFERRAL_ORDER_QUANTITY'])
        {
            ...
            if (!Validate::isLoadedObject($cartRule))
                return false;
            $this->smarty->assign(array('discount' => ReferralCode::displayDiscount($cartRule->reduction_percent ? $cartRule->reduction_percent : $cartRule->reduction_amount, $cartRule->reduction_percent ? 1 : 2, new Currency((int)$params['objOrder']->id_currency)), 'sponsor_firstname' => $sponsor->firstname, 'sponsor_lastname' => $sponsor->lastname));
    ...
    */
    public function hookUpdateOrderStatus($params)
    {
        ...
        if ((int)$orderState->logable AND $nbOrdersCustomer >= (int)$this->_configuration['REFERRAL_ORDER_QUANTITY'] AND $referralprogram->registerDiscountForSponsor((int)$order->id_currency))
        {
            ...
            $discount_display = ReferralCode::displayDiscount( (float) $cartRule->reduction_percent ? (float) $cartRule->reduction_percent : (int) $cartRule->reduction_amount,  (float) $cartRule->reduction_percent ? 1 : 2, $currency);            
            ...
            Mail::Send((int)$order->id_lang, 'referralprogram-congratulations', Mail::l('Congratulations!', (int)$order->id_lang), $data, $sponsor->email, strval(Configuration::get('PS_SHOP_EMAIL')), strval(Configuration::get('PS_SHOP_NAME')), NULL, NULL, dirname(__FILE__).'/mails/');
            return true;
        }
        return false;
    }
        ...
        $fields_form_1 = array(
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        'type' => 'radio',
                        'label' => $this->l('Voucher type sponsorer :'),
                        'name' => 'discount_type_sponsorer',
                        'class' => 't',
                        'values' => array(
                            array(
                                'id' => 'discount_type1',
                    ...
                    array(
                        ...
                        'name' => 'discount_value_percentage_sponsorer',
                        'suffix' => '%'
                    ),
                    array(
                        'type' => 'discount_value',
                        'label' => $this->l('Voucher amount'),
                        'name' => 'discount_value_sponsorer'
                    ),
                    array(
                        'type' => 'select',
					...
					array(
                        'type' => 'radio',
                        'label' => $this->l('Voucher type for sponsored :'),
                        'name' => 'discount_type_sponsored',
                        'class' => 't',
                        'values' => array(
                            array(
                                'id' => 'discount_type1',
                    ...
                    array(
                        'type' => 'text',
                        'label' => $this->l('Percentage'),
                        'name' => 'discount_value_percentage_sponsored',
                        'suffix' => '%'
                    ),
                    array(
                        'type' => 'discount_value',
                        'label' =>     $this->l('Voucher amount'),
                        'name' => 'discount_value_sponsored'
                    ),
                    array(
                        'type' => 'select',
                        'label' =>     $this->l('Voucher tax'),
    ...
    public function getConfigFieldsValues()
    {
        $fields_values = array(
            ...
            'discount_type_sponsored' => Tools::getValue('discount_type_sponsored', Configuration::get('REFERRAL_DISCOUNT_TYPE_SPONSORED')),
			'discount_type_sponsorer' => Tools::getValue('discount_type_sponsorer', Configuration::get('REFERRAL_DISCOUNT_TYPE_SPONSORER')),
            'nb_friends' => Tools::getValue('nb_friends', Configuration::get('REFERRAL_NB_FRIENDS')),
            'discount_value_percentage_sponsored' => Tools::getValue('discount_value_percentage_sponsored', Configuration::get('REFERRAL_PERCENTAGE_SPONSORED')),
			'discount_value_percentage_sponsorer' => Tools::getValue('discount_value_percentage_sponsorer', Configuration::get('REFERRAL_PERCENTAGE_SPONSORER')),
        ...
        foreach ($currencies as $currency)
            $fields_values['discount_value_sponsored'][$currency['id_currency']] = Tools::getValue('discount_value_sponsored['.(int)$currency['id_currency'].']', Configuration::get('REFERRAL_DISCOUNT_VALUE_SPONSORED_'.(int)$currency['id_currency']));

		foreach ($currencies as $currency)
            $fields_values['discount_value_sponsorer'][$currency['id_currency']] = Tools::getValue('discount_value_sponsorer['.(int)$currency['id_currency'].']', Configuration::get('REFERRAL_DISCOUNT_VALUE_SPONSORER_'.(int)$currency['id_currency']));
