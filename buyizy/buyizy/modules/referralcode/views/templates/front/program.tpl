...
// <![CDATA[
	ThickboxI18nClose = "{l s='Close' mod='referralcode'}";
	ThickboxI18nOrEscKey = "{l s='or Esc key' mod='referralcode'}";
	tb_pathToImage = "{$img_ps_dir}loadingAnimation.gif";
	//]]>
</script>

{capture name=path}<a href="{$link->getPageLink('my-account', true)|escape:'html'}" title="{l s='Manage my account' mod='referralcode'}" rel="nofollow">{l s='My account' mod='referralcode'}</a><span class="navigation-pipe">{$navigationPipe}</span>{l s='Referral Code'}{/capture}


<h2>{l s='Referral program' mod='referralcode'}</h2>
...
<ul class="idTabs clearfix">
	<li><a href="#idTab1" title="{l s='Sponsor my friends' mod='referralcode'}" {if $activeTab eq 'sponsor'}class="selected"{/if} rel="nofollow">{l s='Sponsor my friends' mod='referralcode'}</a></li>
	<li><a href="#idTab2" title="{l s='List of pending friends' mod='referralcode'}" {if $activeTab eq 'pending'}class="selected"{/if} rel="nofollow">{l s='Pending friends' mod='referralcode'}</a></li>
	<li><a href="#idTab3" title="{l s='List of friends I sponsored' mod='referralcode'}" {if $activeTab eq 'subscribed'}class="selected"{/if} rel="nofollow">{l s='Friends I sponsored' mod='referralcode'}</a></li>
</ul>
<div class="sheets">

	<div id="idTab1">
		<p class="bold">
			<strong>{l s='Get a discount of %1$s for you and your friends by recommending this Website.' sprintf=[$discount] mod='referralcode'}</strong>
		</p>
		{if $canSendInvitations}
			<p>
				...
				{/if},
				{l s='he or she will receive a %1$s voucher and you will receive your own voucher worth %1$s.' sprintf=[$discount] mod='referralcode'}
			</p>
			<form method="post" action="{$link->getModuleLink('referralcode', 'code', [], true)|escape:'html'}" class="std">
				<table class="std">
				<thead>
					<tr>
				...
				<p class="checkbox">
					...
					<a href="{$link->getModuleLink('referralcode', 'rules', ['height' => '500', 'width' => '400'], true)|escape:'html'}" class="thickbox" title="{l s='Conditions of the referral program' mod='referralcode'}" rel="nofollow">{l s='Read conditions.' mod='referralcode'}</a>
				</p>
				<p class="see_email">
					{l s='Preview' mod='referralcode'}
					{assign var="file" value="{$lang_iso}/referralcode-invitation.html"}
					<a href="{$link->getModuleLink('referralcode', 'email', ['height' => '500', 'width' => '600', 'mail' => {$file}], true)|escape:'html'}" class="thickbox" title="{l s='Invitation e-mail' mod='referralcode'}" rel="nofollow">{l s='the default e-mail' mod='referralcode'}</a> {l s='that will be sent to your friend(s).' mod='referralcode'}
				</p>
				<p class="submit">
					<input type="submit" id="submitSponsorFriends" name="submitSponsorFriends" class="button_large" value="{l s='Validate' mod='referralcode'}" />
				</p>
			</form>
		{else}
			<p class="warning">
	...
	{if $pendingFriends AND $pendingFriends|@count > 0}
		...
		<form method="post" action="{$link->getModuleLink('referralcode', 'code', [], true)|escape:'html'}" class="std">
			...
			{foreach from=$pendingFriends item=pendingFriend name=myLoop}
				<tr>
					<td>
						<input type="checkbox" name="friendChecked[{$pendingFriend.id_referralcode}]" id="friendChecked[{$pendingFriend.id_referralcode}]" value="1" />
					</td>
					<td>
						<label for="friendChecked[{$pendingFriend.id_referralcode}]">{$pendingFriend.lastname|substr:0:22}</label>
					</td>
			...
			<p class="submit">
				<input type="submit" value="{l s='Remind my friend(s)' mod='referralcode'}" name="revive" id="revive" class="button_large" />
			</p>
		</form>
		{else}
			<p class="warning">
				{if $subscribeFriends AND $subscribeFriends|@count > 0}
					{l s='You have no pending invitations.' mod='referralcode'}
				{else}
					{l s='You have not sponsored any friends yet.' mod='referralcode'}
				{/if}
			</p>
		{/if}
	</div>
...
<ul class="footer_links">
	<li>
		<a href="{$link->getPageLink('my-account', true)|escape:'html'}" title="{l s='Back to Your Account' mod='referralcode'}" rel="nofollow">
		<img src="{$img_dir}icon/my-account.gif" alt="" class="icon" /> {l s='Back to Your Account' mod='referralcode'}</a>
	</li>
	<li class="f_right"><a href="{$base_dir}" title="{l s='Home' mod='referralcode'}"><img src="{$img_dir}icon/home.gif" alt="" class="icon" /> {l s='Home' mod='referralcode'}</a></li>
</ul>
