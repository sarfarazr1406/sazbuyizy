...
class TmHomeSlider extends Module
{
	...
	{
		/* Adds Module */
		if (parent::install() && $this->registerHook('displayHeader') && $this->registerHook('displayHome') && $this->registerHook('actionShopDataDuplication'))
		{
			/* Sets up configuration */
			$res = Configuration::updateValue('HOMESLIDER_WIDTH', '706');
			$res &= Configuration::updateValue('HOMESLIDER_SPEED', '398');
	...
	private function installSamples()
	{
		...
		for ($i = 1; $i <= 3; ++$i)
		{
			...
			foreach ($languages as $language)
			{
				...
				<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin tristique in tortor et dignissim. 
				Quisque non tempor leo. Maecenas egestas sem elit</p>
				<p><button class="btn btn-default" type="button">Shop now !</button></p>';
				$slide->legend[$language['id_lang']] = 'mainbanner-'.$i;
				$slide->url[$language['id_lang']] = '#';
				$slide->image[$language['id_lang']] = 'mainbanner-'.$i.'.jpg';
			}
			$slide->add();
		}
	}
	...
	private function _postprocess()
	{
		...
		elseif (Tools::isSubmit('submitSlide'))
		{
			...
			foreach ($languages as $language)
			{
				...
				if (isset($_FILES['image_'.$language['id_lang']]) && isset($_FILES['image_'.$language['id_lang']]['tmp_name']) && !empty($_FILES['image_'.$language['id_lang']]['tmp_name']) && !empty($imagesize) && in_array(Tools::strtolower(Tools::substr(strrchr($imagesize['mime'], '/'), 1)),
				array('jpg','gif','jpeg','png')) && in_array($type, array('jpg', 'gif', 'jpeg', 'png')))
				{
	...
	public function hookdisplayHeader($params)
	{
		if (!isset($this->context->controller->php_self) || $this->context->controller->php_self != 'index')
			return;
		$this->context->controller->addCSS($this->_path.'css/tmhomeslider.css');
		$this->context->controller->addJS($this->_path.'js/tmhomeslider.js');
