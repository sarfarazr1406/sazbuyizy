                    ...
                    {if $slide.active}
                        <li class="tmhomeslider-container" id="slide_{$item}">
                            <a href="{$slide.url|escape:'html':'UTF-8'}" title="{$slide.legend|escape:'html':'UTF-8'}">
                                <img src="{$link->getMediaLink("`$smarty.const._MODULE_DIR_`tmhomeslider/img/`$slide.image|escape:'htmlall':'UTF-8'`")}"
                                     alt="{$slide.legend|escape:'htmlall':'UTF-8'}"/>
                            </a>
                            {if isset($slide.description) && trim($slide.description) != ''}
                                <div class="tmhomeslider-description">{$slide.description}</div>
                            {/if}
                        </li>
					{/if}
