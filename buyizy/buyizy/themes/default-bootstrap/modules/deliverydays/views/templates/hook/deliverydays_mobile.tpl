...
<div class="addresses" style="margin-top: 20px;">
	<div id="ordermsg" class="clearfix">
		...
		<div class="textarea" style="float: left; margin-bottom: 1em; padding-left: 1.4em;">
			<input type="hidden" id="deliverydays_day" name="deliverydays_day" value="{$selected_day.date|escape:'htmlall':'UTF-8'}"/>
			...
			<select id="deliverydays_timeframe" name="deliverydays_timeframe" disabled>
				{foreach from=$timeframe item=frame name="delivery_loop"}
					...
					{if $slot_no == 3}
					    ...
					    {/if}
					{/if}
					{assign value=$slot_no+1 var='slot_no'}
				{/foreach}
			</select>
		</div>
...
{literal}<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#deliverydays_picker').parents('div[data-role="collapsible"]').attr( "data-collapsed", "false" );
	});
</script>
<style type="text/css">
